...
$('#plannerForm').onsubmit=async e=>{e.preventDefault();const cid=$('#planClass').value,sid=$('#planSport').value;if(!cid||!sid)return;const sp=st.sports.find(x=>x.id===sid),cl=st.classes.find(x=>x.id===cid);$('#plannerMsg').textContent='Controllo l’orario della classe…';const{data:slots,error:slotErr}=await db.from('pe_class_timetable_slots').select('id,weekday,start_time,end_time').eq('class_id',cid).eq('active',true);if(slotErr)return $('#plannerMsg').textContent=slotErr.message;if(!(slots||[]).length)return $('#plannerMsg').textContent='Dati insufficienti: questa classe non ha ancora giorni e orari di lezione. Apri Classi → modifica la classe → Orario settimanale.';$('#plannerMsg').textContent='Genero lezioni complete dall’archivio verificato, in progressione…';let{data:m,error}=await db.from('pe_sport_modules').insert({owner_id:st.user.id,class_id:cid,sport_id:sid,module_type:'automatic',title:`${sp.name} · ${cl.name}`,start_date:$('#planStart').value,planned_weeks:+$('#planWeeks').value,lesson_duration_min:+$('#planMinutes').value,level_override:$('#planLevel').value?+$('#planLevel').value:null,progression_mode:'progressive',status:'planned'}).select().single();if(error)return $('#plannerMsg').textContent=error.message;let{error:er}=await db.rpc('pe_generate_module_plan',{p_module_id:m.id,p_regenerate:false});if(er)return $('#plannerMsg').textContent=er.message;try{await applyTimetableTimesToModule(m.id,cid)}catch(timeErr){console.warn('Orari modulo non applicati',timeErr)}$('#plannerMsg').textContent='Programmazione completa generata: ogni lezione contiene attività reali dell’archivio, in progressione e senza ripetizioni finché esistono alternative.';toast('Modulo generato');await loadCore()}
let replaceCtx=null;
...
async function loadRankings(){
  const cid=$('#rankingClass').value||st.classes[0]?.id,sex=$('#rankingSex').value;if(!cid)return;
  $('#rankingOverview').innerHTML=metricCard('◌','Caricamento','…','Analizzo i dati della classe','blue');$('#rankingPodium').innerHTML='';
  let{data:t,error:te}=await db.from('pe_motor_test_total_rankings').select('*').eq('class_id',cid).eq('sex',sex).order('total_place');if(te){toast(te.message);t=[]}
  t=t||[];const pointsAvg=avg(t.map(x=>x.total_points)),leader=t[0];
  $('#totalRankingMeta').textContent=t.length?`${t.length} atleti`:'Nessun dato';$('#totalRanking').innerHTML=t.length?rankingRows(t,'total',pointsAvg):emptyPremium('Classifica generale ancora vuota');
  let{data:sess}=await db.from('pe_motor_test_sessions').select('id,test_id,session_date,title').eq('class_id',cid).order('session_date',{ascending:false}).limit(1);
  if(!sess?.length){$('#singleRanking').innerHTML=emptyPremium('Nessun test registrato');$('#singleRankingTitle').textContent='Classifica test';$('#singleRankingMeta').textContent='—';$('#rankingOverview').innerHTML=[metricCard('◎','Partecipanti',t.length||'—','atleti con punteggio','blue'),metricCard('Σ','Media punti',pointsAvg===null?'—':fmtNum(pointsAvg,1),'classifica generale','good'),metricCard('♛','Leader',leader?`${leader.total_points} pt`:'—',leader?`${leader.first_name} ${leader.last_name}`:'Nessun dato','gold'),metricCard('↗','Trend','—','serve almeno un test','violet')].join('');$('#rankingPodium').innerHTML=podiumHTML(t,'total');return}
  const latest=sess[0];let{data:r,error:re}=await db.from('pe_motor_test_rankings').select('*').eq('session_id',latest.id).eq('sex',sex).order('place');if(re){toast(re.message);r=[]}r=r||[];
  const testMean=avg(r.map(x=>x.result_value)),best=r[0],testName=r[0]?.test_name||latest.title||'Ultimo test',unit=r[0]?.unit||'',direction=r[0]?.result_direction||'higher_better';
  $('#singleRankingTitle').textContent=testName;$('#singleRankingMeta').textContent=new Date(latest.session_date+'T12:00:00').toLocaleDateString('it-IT');$('#singleRanking').innerHTML=r.length?rankingRows(r,'result',testMean):emptyPremium('Nessun risultato nell’ultimo test');$('#rankingPodium').innerHTML=podiumHTML(r.length?r:t,r.length?'result':'total');
  ...
  if(prevSess?.length&&r.length){let{data:pr}=await db.from('pe_motor_test_rankings').select('result_value').eq('session_id',prevSess[0].id).eq('sex',sex);const prevMean=avg((pr||[]).map(x=>x.result_value));if(prevMean!==null&&testMean!==null){const raw=direction==='lower_better'?prevMean-testMean:testMean-prevMean;const pct=prevMean?Math.abs(raw/prevMean*100):0;trendText=`${raw>=0?'↗':'↘'} ${raw>=0?'+':''}${fmtNum(raw)}`;trendDetail=`${raw>=0?'miglioramento':'calo'} medio · ${fmtNum(pct,1)}% vs precedente`;trendClass=raw>=0?'good':'violet'}}
  $('#rankingOverview').innerHTML=[metricCard('◎','Partecipanti',r.length,`${sex==='F'?'femmine':'maschi'} nell’ultimo test`,'blue'),metricCard('≈','Media gruppo',testMean===null?'—':`${fmtNum(testMean)} ${unit}`,testName,'good'),metricCard('♛','Miglior risultato',best?`${fmtNum(best.result_value)} ${unit}`:'—',best?`${best.first_name} ${best.last_name}`:'Nessun dato','gold'),metricCard('↗','Trend classe',trendText,trendDetail,trendClass)].join('');
}
...
$('#addExtraLesson').onclick=()=>{if(!requireSchoolYear('Crea prima l’anno scolastico per inserire una lezione extra.'))return;$('#extraLessonMsg').textContent='';if(!$('#extraDate').value)$('#extraDate').value=localISODate(new Date());$('#extraLessonModal').showModal()};
$('#extraLessonForm').onsubmit=async e=>{e.preventDefault();const msg=$('#extraLessonMsg'),cid=$('#extraClass').value,date=$('#extraDate').value,start=$('#extraStart').value,end=$('#extraEnd').value;if(!cid||!date||!start||!end){msg.textContent='Completa classe, data e orario.';return}const mins=minutesBetween(start,end);if(mins<=0){msg.textContent='L’ora di fine deve essere successiva all’ora di inizio.';return}const cl=st.classes.find(x=>x.id===cid),custom=$('#extraTitle').value.trim(),title=custom||`Lezione extra · ${cl?.name||'Classe'}`;msg.textContent='Inserisco la lezione nel calendario…';const{error}=await db.from('pe_lessons').insert({owner_id:st.user.id,module_id:null,class_id:cid,sport_id:null,lesson_date:date,sequence_no:1,title,duration_min:mins,generation_mode:'manual',status:'planned',teacher_notes:$('#extraNotes').value.trim()||null,start_time:start,end_time:end,is_extra:true});if(error){msg.textContent='Errore: '+error.message;return}$('#extraLessonModal').close();e.currentTarget.reset();$('#extraDate').value=localISODate(new Date());toast('Lezione extra inserita');await loadCore();renderCalendar()};
function renderSettings(){
  ...
  const build=$('#buildVersion');if(build)build.textContent='Versione 4.5';
}

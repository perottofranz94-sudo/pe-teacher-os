...
async function openClass(id){
  ...
  await Promise.all([renderLevelGrid(id),loadClassTimetable(id)]);$('#classModal').showModal();
}
...
function renderModules(){$('#moduleList').innerHTML=st.modules.slice(0,18).map(x=>listItem(x.title,`${x.pe_classes?.name||''} · ${x.planned_weeks} lezioni`, `<span class="chip">${x.status}</span>`)).join('')||listItem('Nessun modulo','Creane uno dal generatore')}
...
function metricCard(icon,label,value,detail,cls=''){return `<article class="performance-card ${cls}"><span class="metric-icon">${icon}</span><small>${esc(label)}</small><b>${value}</b><em>${esc(detail||'')}</em></article>`}
...
function podiumHTML(rows,mode='result'){if(!rows?.length)return '';const top=rows.slice(0,3),order=[top[1],top[0],top[2]],classes=['second','first','third'],medals=['🥈','🥇','🥉'];return order.map((x,i)=>x?`<article class="podium-card ${classes[i]}"><div class="podium-medal">${medals[i]}</div><div class="podium-place">${x.place||x.total_place}° POSTO</div><div class="podium-name">${esc(x.first_name)} ${esc(x.last_name)}</div><div class="podium-score">${mode==='total'?`${x.total_points} pt`:`${fmtNum(x.result_value)} ${esc(x.unit||'')}`}</div><div class="podium-detail">${mode==='total'?`${x.scored_results} risultati a punti`:`${x.points} punti graduatoria`}</div><div class="podium-bar"></div></article>`:'').join('')}
function rankingRows(rows,mode='result',mean=null){return (rows||[]).map(x=>{const place=mode==='total'?x.total_place:x.place,val=mode==='total'?x.total_points:Number(x.result_value),unit=mode==='total'?'pt':x.unit||'',diff=mean===null?null:val-mean;return `<div class="premium-rank-row ${place===1?'top1':''}"><div class="rank-pos">${place}</div><div class="rank-athlete"><strong>${esc(x.first_name)} ${esc(x.last_name)}</strong><small>${mode==='total'?`${x.scored_results} test a punti`:`${esc(x.test_name||'Test motorio')}`}</small></div><div class="rank-value"><b>${fmtNum(val)} ${esc(unit)}</b>${diff===null?'':`<span class="rank-delta ${diff<0?'below':''}">${diff>=0?'+':''}${fmtNum(diff)} dalla media</span>`}</div></div>`}).join('')}
...
async function loadRankings(){
  ...
  $('#rankingOverview').innerHTML=metricCard('◌','Caricamento','…','Analizzo i dati della classe','blue');$('#rankingPodium').innerHTML='';
  ...
  const pointsAvg=avg(t.map(x=>x.total_points)),leader=t[0];
  $('#totalRankingMeta').textContent=t.length?`${t.length} alunni`:'Nessun dato';$('#totalRanking').innerHTML=t.length?rankingRows(t,'total',pointsAvg):emptyPremium('Classifica generale ancora vuota');
  ...
  if(!sess?.length){
    $('#singleRanking').innerHTML=emptyPremium(tid?'Nessuna rilevazione di questo test per la classe':'Nessun test registrato');$('#singleRankingTitle').textContent=tid?(st.tests.find(x=>x.id===tid)?.name||'Classifica test'):'Classifica test';$('#singleRankingMeta').textContent='—';
    $('#rankingOverview').innerHTML=[metricCard('◎','Partecipanti',t.length||'—',sex==='F'?'alunne con punteggio':sex==='M'?'alunni con punteggio':'alunni con punteggio','blue'),metricCard('Σ','Media punti',pointsAvg===null?'—':fmtNum(pointsAvg,1),'classifica generale','good'),metricCard('♛','Leader',leader?`${leader.total_points} pt`:'—',leader?`${leader.first_name} ${leader.last_name}`:'Nessun dato','gold'),metricCard('⌁','Filtro test',tid?(st.tests.find(x=>x.id===tid)?.name||'Selezionato'):'Tutti','nessuna rilevazione disponibile','violet')].join('');$('#rankingPodium').innerHTML=podiumHTML(t,'total');return
  }
  ...
  const testMean=avg(r.map(x=>x.result_value)),best=r[0],testName=r[0]?.test_name||latest.title||'Test',unit=r[0]?.unit||st.tests.find(x=>x.id===latest.test_id)?.unit||'';
  $('#singleRankingTitle').textContent=testName;$('#singleRankingMeta').textContent=new Date(latest.session_date+'T12:00:00').toLocaleDateString('it-IT');$('#singleRanking').innerHTML=r.length?rankingRows(r,'result',testMean):emptyPremium('Nessun risultato nella selezione');$('#rankingPodium').innerHTML=podiumHTML(r.length?r:t,r.length?'result':'total');
  let trendText='Prima rilevazione',trendDetail='nessun confronto precedente',trendClass='violet';
  let{data:prevSess}=await db.from('pe_motor_test_sessions').select('id,session_date').eq('class_id',cid).eq('test_id',latest.test_id).lt('session_date',latest.session_date).order('session_date',{ascending:false}).limit(1);
  if(prevSess?.length&&r.length){let pq=db.from('pe_motor_test_rankings').select('result_value').eq('session_id',prevSess[0].id);if(sex)pq=pq.eq('sex',sex);let{data:pr}=await pq;const prevMean=avg((pr||[]).map(x=>x.result_value));if(prevMean!==null&&testMean!==null){const raw=direction==='lower_better'?prevMean-testMean:testMean-prevMean;const pct=prevMean?Math.abs(raw/prevMean*100):0;trendText=`${raw>=0?'↗':'↘'} ${raw>=0?'+':''}${fmtNum(raw)}`;trendDetail=`${raw>=0?'miglioramento':'calo'} medio · ${fmtNum(pct,1)}% vs precedente`;trendClass=raw>=0?'good':'violet'}}
  const groupLabel=sex==='F'?'femmine':sex==='M'?'maschi':'classe intera';
  $('#rankingOverview').innerHTML=[metricCard('◎','Partecipanti',r.length,`${groupLabel} · ${testName}`,'blue'),metricCard('≈','Media gruppo',testMean===null?'—':`${fmtNum(testMean)} ${unit}`,groupLabel,'good'),metricCard('♛','Miglior risultato',best?`${fmtNum(best.result_value)} ${unit}`:'—',best?`${best.first_name} ${best.last_name}`:'Nessun dato','gold'),metricCard('↗','Trend classe',trendText,trendDetail,trendClass)].join('');
}
...
async function generateSingleAutomaticLesson(lessonId,classId,sportId,totalMinutes){const [{data:cl,error:ce},{data:lv,error:le},{data:ex,error:ee}]=await Promise.all([db.from('pe_classes').select('general_level,student_count').eq('id',classId).single(),db.from('pe_class_sport_levels').select('level').eq('class_id',classId).eq('sport_id',sportId).maybeSingle(),db.from('pe_exercises').select('id,name,difficulty,duration_min,duration_max,category_id,fun_score,decision_making,idle_time,max_players').eq('sport_id',sportId).eq('active',true).eq('audit_status','VERIFIED').limit(500)]);if(ce||le||ee)throw(ce||le||ee);const level=Math.max(1,Math.min(5,Number(lv?.level||cl?.general_level||2))),all=ex||[];if(all.length<3)throw new Error('Archivio insufficiente per generare una lezione completa.');let chosen=[];const take=(score)=>{const avail=all.filter(x=>!chosen.some(c=>c.id===x.id)).sort((a,b)=>score(a)-score(b));const x=avail[0];if(x)chosen.push(x);return x};const activation=take(x=>Math.abs((x.difficulty||3)-Math.max(1,level-1))*10-(x.fun_score||3)+(x.idle_time||3));let mainCount=totalMinutes<=60?2:totalMinutes<=90?3:4;let mains=[];for(let i=0;i<mainCount;i++){const target=Math.max(1,Math.min(5,level+(i>=Math.ceil(mainCount/2)?0:-1)));mains.push(take(x=>Math.abs((x.difficulty||3)-target)*10+(chosen.some(c=>c.category_id&&c.category_id===x.category_id)?4:0)-(x.decision_making||3)*.4))}const final=take(x=>Math.abs((x.difficulty||3)-Math.min(5,level+1))*7-(x.decision_making||3)*2-(x.fun_score||3));const actMin=Math.min(15,Math.max(8,Math.round(totalMinutes*.13))),closeMin=Math.min(8,Math.max(5,Math.round(totalMinutes*.05))),finalMin=Math.min(30,Math.max(15,Math.round(totalMinutes*.27))),mainTotal=totalMinutes-actMin-closeMin-finalMin,each=Math.floor(mainTotal/mainCount),rows=[];rows.push({e:activation,phase:'activation',dur:actMin,reason:'Attivazione coerente con il livello della classe e a basso tempo inattivo.'});mains.forEach((x,i)=>rows.push({e:x,phase:'main',dur:i===mainCount-1?mainTotal-each*(mainCount-1):each,reason:'Parte centrale scelta dall’archivio verificato con difficoltà progressiva.'}));rows.push({e:final,phase:'final',dur:finalMin,reason:'Applicazione finale più decisionale e vicina alla situazione reale.'});const payload=rows.map((r,i)=>({owner_id:st.user.id,lesson_id:lessonId,exercise_id:r.e.id,phase:r.phase,order_no:i+1,duration_min:r.dur,selection_reason:r.reason}));payload.push({owner_id:st.user.id,lesson_id:lessonId,exercise_id:null,phase:'closing',order_no:payload.length+1,duration_min:closeMin,custom_title:'Chiusura e feedback',custom_explanation:'Recupero materiale, breve feedback e riepilogo degli apprendimenti.',selection_reason:'Chiusura flessibile della lezione.'});const{error}=await db.from('pe_lesson_exercises').insert(payload);if(error)throw error}
...
function renderSettings(){
  ...
  const build=$('#buildVersion');if(build)build.textContent='Versione 4.8';
}

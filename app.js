...
async function enter(user){const isOwner = user?.app_metadata?.app_role === "owner" || user?.app_metadata?.is_owner === true;

const ownerNavBtn = document.getElementById("ownerNavBtn");
if (ownerNavBtn) {
  ownerNavBtn.classList.toggle("hidden", !isOwner);
}st.user=user;$('#userMail').textContent=user.email||'';$('#authView').classList.add('hidden');$('#appView').classList.remove('hidden');await syncFromCloud()}
